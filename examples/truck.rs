...
//!
//! Run:  cargo run --example truck --release --features "raytracer image"
...
const TRUCK_BLACK: (u8, u8, u8) = (0, 0, 0);
...
fn load_image_assets() -> Vec<ImageAsset> {
    ...
    let candidates: &[(&str, &str, &'static str)] = &[
        ...
        ("src/image/truck.png", "./src/image/truck.jpg", "TRUCK"),
    ];
    let mut out = Vec::new();
...
fn build_scene(w: i32, h: i32) -> Scene {
    ...
    let truck_h = (middle_h * 2 / 3).clamp(8, 26);
    let truck_gap = 2;
    let available = (w - 2 * truck_gap).max(12);
    let truck_w = (available / 3).clamp(12, 60);
    let row_w = truck_w * 3 + 2 * truck_gap;
    let ui_truck_mid_y = middle_top + (middle_h - truck_h) / 2;
    let row_x = ((w - row_w) / 2).max(1);
    let ui_truck_left = Rect {
        x: row_x,
        y: ui_truck_mid_y,
        w: truck_w,
        h: truck_h,
    };
    let ui_truck_center = Rect {
        x: row_x + truck_w + truck_gap,
        y: ui_truck_mid_y,
        w: truck_w,
        h: truck_h,
    };
    let ui_truck_right = Rect {
        x: row_x + 2 * (truck_w + truck_gap),
        y: ui_truck_mid_y,
        w: truck_w,
        h: truck_h,
    };
    ...
    if !assets.is_empty() {
        let truck_idx = assets.iter().position(|a| a.name == "TRUCK").unwrap_or(0);
        for &i in &sorted_by_area {
            let r = zones[i].base_rect;
            if r.w < 10 || r.h < 6 {
                continue;
            }
            zones[i].formation = Formation::ImagePanel { asset: truck_idx };
        }
    }
    ...
    let truck_idx = if assets.is_empty() {
        0
    } else {
        assets.iter().position(|a| a.name == "TRUCK").unwrap_or(0)
    };
    push_ui(
        ui_truck_left,
        Formation::ImagePanel { asset: truck_idx },
        &mut tap_accum,
    );
    push_ui(
        ui_truck_center,
...
fn paint_image_panel(
    ...
    let is_truck = asset.name == "TRUCK";
    let color = if is_truck {
        TRUCK_BLACK
    } else {
        color_for(zone.side)
    };
        ...
        for rx in 0..zw {
            ...
            let (ch, intensity, fg_override) = if on_front {
                ...
                (
                    ...
                    1.0,
                    Some(if is_truck { TRUCK_BLACK } else { (255, 50, 50) }),
                )
            } else if h & 0x7f == 0 {
                let blocks: &[char] = &['█', '▓', '▒', '░'];
    ...
    let label_color = if is_truck { TRUCK_BLACK } else { (255, 50, 50) };
