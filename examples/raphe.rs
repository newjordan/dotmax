...
fn load_image_assets() -> Vec<ImageAsset> {
    ...
    let candidates: &[(&str, &str, &'static str)] = &[
        ...
        ("src/image/raphe.jpg", "./src/image/raphe.jpg", "RAPH"),
    ];
    let mut out = Vec::new();
...
fn build_scene(w: i32, h: i32) -> Scene {
    ...
    if !assets.is_empty() {
        let raphe_idx = assets.iter().position(|a| a.name == "RAPH").unwrap_or(0);
    ...
    let raphe_idx = if assets.is_empty() {
        ...
        assets.iter().position(|a| a.name == "RAPH").unwrap_or(0)
    };
    push_ui(
        ui_raphe_left,
