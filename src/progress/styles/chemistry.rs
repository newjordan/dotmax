...
impl ProgressStyle for PhScale {
    ...
    fn render(&self, grid: &mut BrailleGrid, ctx: &BarContext) -> Result<(), DotmaxError> {
        ...
        // --- Sliding indicator: a bright full-column of dots ---
        // pH = eased × 14, position = eased * (w-1)
        let indicator_x = (ctx.eased * (w - 1) as f32).round() as usize;
        // Draw indicator as a solid vertical dot line
        for y in 0..h {
            draw::dot(grid, indicator_x.min(w - 1), y);
        }
        // Also draw one cell to each side (thicker marker)
        if indicator_x > 0 {
